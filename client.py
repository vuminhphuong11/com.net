...
ALLOWED_MINE_PREFIXES = ['image/', 'audio/', 'video/', 'text/', 'application/pdf']

def send_message_with_length(sock, message):
    try:
        msg_bytes = message.encode('utf-8')
        length = len(msg_bytes)
        sock.send(length.to_bytes(4, 'big') + msg_bytes)
        return True
...
def is_allowed_filetype(filepath):
    ...
    if mime_type is None:
        return False
    return any(mime_type.startswith(prefix) for prefix in ALLOWED_MINE_PREFIXES)
